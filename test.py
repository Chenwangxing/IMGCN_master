...
def test(model, loader_test, KSTEPS=20):
    ...
    for batch in loader_test:
        ...
        # V_obs torch.Size([1(batchsize), 8(obs), 2(行人数目), 3(特征（对应时间，x坐标，y坐标）)])

        # identity
        V_pred = model(V_obs, identity, obs_traj)  # A_obs <8, #, #>
        # V_pred torch.Size([12(pred), 2(nums), 5(feat特征)])
...
def main():
    ...
    dataset = ['IMGCN-ETHUCY/eth', 'IMGCN-ETHUCY/hotel', 'IMGCN-ETHUCY/univ', 'IMGCN-ETHUCY/zara1', 'IMGCN-ETHUCY/zara2']
